...
# use a JDK (>= 21) compatible with the one that was used to compile OpenCV and is compatible with the Kotlin JVM and JavaFX
...
if [ ! -f "openapi-generator-cli.jar" ]; then
	./download_openapi_generator_cli.sh || exit 1
fi

# ----------------------------------------------------------------------------------------------------------------------

"${TMP_JAVA_PATH}java" -jar openapi-generator-cli.jar "$@"
